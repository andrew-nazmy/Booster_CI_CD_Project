pipeline {
    ...
    stages {
        ...
        stage('Build') {
                steps {
                    sh 'docker build -f Dockerfile . -t nazmy123/django_appdev:v1.0'
                      }
        ...
        stage('Push') {
            steps {
                ...
                sh 'docker push nazmy123/django_appdev:v1.0'
                }
            }
        }
         stage('Deploy') {
            steps {
                sh 'docker run -d -p 4000:3000 nazmy123/django_appdev:v1.0'
            }  
    }
  }
post {
